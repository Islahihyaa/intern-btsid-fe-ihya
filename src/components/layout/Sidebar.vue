<template>
  ...
  >
    <div class="flex flex-col">
        ...
        <div class="overflow-y-auto">
          <li v-for="board in $state.boards" :key="board.boardId" class="m-2">
            <router-link :to="{ path: `/board/${board.boardId}` }">
              <div @click="showDetailBoard(board.boardId)" class="m-2">
                <div class="text-md">
                  {{ board.boardTitle }}
                </div>
              </div>
      ...
      <ul v-if="$state.sharedBoards.length > 0" class="menu w-72 flex-1">
        <div class="py-4 px-2 text-white text-sm mb-1">
          <p>Share Boards</p>
        </div>
        <div class="overflow-y-auto">
          ...
          >
            ...
            >
              <div
                @click="showDetailBoard(sharedBoard.board.boardId)"
                class="m-2"
              >
                <div class="text-md">
